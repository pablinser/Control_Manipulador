%% trabajoRob2.m
% Este fichero se debe ejecutar tras la ejecución de parComputado.slx, ya
...
figure
subplot (3,2,1)
plot (time(2:end),qd(1:end-1,1)-q(2:end,1))
title ('theta1')
ylabel ('Error 1 (rads)')
xlabel('t (s)')

subplot (3,2,2)
plot (time(2:end),qd(1:end-1,2)-q(2:end,2))
title ('theta2')
ylabel ('Error 2 (rads)')
xlabel('t (s)')
...
subplot (3,2,3)
plot (time(2:end),qd(1:end-1,3)-q(2:end,3))
title ('theta3')
ylabel ('Error 3 (rads)')
xlabel('t (s)')
...
subplot (3,2,4)
plot (time(2:end),qd(1:end-1,4)-q(2:end,4))
title ('theta4')
ylabel ('Error 4 (rads)')
xlabel('t (s)')
...
subplot (3,2,5)
plot (time(2:end),qd(1:end-1,5)-q(2:end,5))
title ('theta5')
ylabel ('Error 5 (rads)')
xlabel('t (s)')
...
subplot (3,2,6)
plot (time(2:end),qd(1:end-1,6)-q(2:end,6))
title ('theta6')
ylabel ('Error 6 (rads)')
xlabel('t (s)')
...
Td = fkine(dh, qd);
Tr = fkine(dh, q);
td = ones(length(Qt(:,1)), 3);
tr = ones(length(Qt(:,1)), 3);
...
figure
plot(time(2:end,:), sqrt(sum((td(1:end-1,:)-tr(2:end,:)).^2,2)))
